...
apt-get update
ACCEPT_EULA=Y apt-get install -y --no-install-recommends libtinfo5 rabbitmq-server mssql-tools mariadb-server default-jre-headless mongodb-org redis-server
...
- "/smi/smi cohort-packager -y /smi.yaml"
